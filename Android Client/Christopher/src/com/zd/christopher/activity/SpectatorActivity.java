...
public class SpectatorActivity extends Activity implements AnyChatBaseEvent 
{
	...
	private void InitSDK() {
		anychatSDK = MainActivity.anyChatSDK;
	}
	...
	private void InitLayout() {
		...
		mEndCallBtn.setBackgroundColor(Color.RED);
		mBtnSpeakCtrl.setVisibility(View.INVISIBLE);
		mBtnCameraCtrl.setVisibility(View.INVISIBLE);
		mImgSwitchVideo.setVisibility(View.INVISIBLE);
		mOtherView.setZOrderOnTop(true);
		
		getActionBar().setDisplayShowHomeEnabled(false);
		getActionBar().setTitle(anychatSDK.GetUserName(userID) + "的直播间");
	...
	Runnable runnable = new Runnable() {
		...
		public void run() {
			try {
				...
				{
					if (videoBitrate <= 0){						
						Toast.makeText(SpectatorActivity.this, "对方视频中断了!", Toast.LENGTH_SHORT).show();
						// 重置下，如果对方退出了，有进去了的情况
				...
				if (mFirstGetAudioBitrate){
					if (audioBitrate <= 0){
						Toast.makeText(SpectatorActivity.this, "对方音频中断了!", Toast.LENGTH_SHORT).show();
						// 重置下，如果对方退出了，有进去了的情况
	...
	protected void onDestroy() {
		super.onDestroy();
		anychatSDK.Logout();
		anychatSDK.Connect("192.168.1.104", 8182);
		anychatSDK.Login(DataBase.getUserName(getApplicationContext()), "");
		handler.removeCallbacks(runnable);
	...
	public void OnAnyChatUserAtRoomMessage(int dwUserId, boolean bEnter) {
		if (!bEnter) {
			if (dwUserId == userID) {
				Toast.makeText(SpectatorActivity.this, "对方已离开！", Toast.LENGTH_SHORT).show();
				userID = 0;
	...
	public void OnAnyChatLinkCloseMessage(int dwErrorCode) {
		...
		// 销毁当前界面
		destroyCurActivity();
		Intent mIntent = new Intent("SpectatorActivity");
		// 发送广播
